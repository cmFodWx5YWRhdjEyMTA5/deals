...
class SurveysController extends CController
{
    ...
    public function exportAction($surveyId = 0, $format = 'csv_columnar')
    {
        ...
        if($format != 'csv_columnar' && $format != 'csv_tabular' && $format != 'csv_regular') $this->redirect('surveys/manage');
        ...
        if($format == 'csv_tabular'){
        ...
        }else if($format == 'csv_regular'){
            $answer_sheet = [];

            $surveys_participant_table = CConfig::get('db.prefix').SurveyParticipants::model()->getTableName();
            $active_participants = CDatabase::init()->select('
                SELECT phone
                FROM '.$surveys_participant_table.'
                WHERE is_active = :active and phone is not null',
                array(
                    ':active'=>1
                )
            );
            ...
            $active_participants = array_map(function($item){
                return $item['phone'];
            }, $active_participants);

            $surveyAnserTableName = CConfig::get('db.prefix').SurveyAnswers::model()->getTableName();
            $surveyAnswers = CDatabase::init()->select('
                SELECT DISTINCT(participant_id),questionnaire_id
                FROM '.$surveyAnserTableName.'
                WHERE survey_id = :survey_id and participant_id in ('.implode(',', $active_participants).')
                ',
                array(
                    ':survey_id'=>$surveyId,
                )
            ...
            $surveyTableName = CConfig::get('db.prefix').Surveys::model()->getTableName();
            $survey = CDatabase::init()->select('
                SELECT name
            ...
            $questionTableName = CConfig::get('db.prefix').SurveyQuestionnaireItems::model()->getTableName();

            $questions = CDatabase::init()->select('
                SELECT id,question_text
                FROM '.$questionTableName.'
                WHERE entity_questionnaire_id = :entity_questionnaire_id and is_active = :is_active',
                array(
                    ':entity_questionnaire_id'=>$surveyAnswers[0]['questionnaire_id'],
                    ':is_active' => 1
                )
            );
            ...
            foreach ($questions as $question) {
                $question_set[$question['id']] = $question['question_text'];
            }


            foreach ($surveyAnswers as $answer) {
                //print_r($answer);
                $participant = SurveysParticipants::model()->find('phone = :phone', array(':phone'=>$answer['participant_id']));
                $temp_answer_sheet['survey_name'] = $survey[0]['name'];
                $temp_answer_sheet['name'] = str_replace(",","",$participant->full_name);
                $temp_answer_sheet['email'] = $participant->email;
                $temp_answer_sheet['phone'] = $participant->phone;

                //print_r($temp_answer_sheet);
                $part_answer = SurveyAnswers::model()->findAll('survey_id =:survey_id and participant_id =:participant_id',[':survey_id'=>$surveyId, ':participant_id' => $answer['participant_id']]);
                foreach ($part_answer as $ques_value) {
                    if($ques_value['questionnaire_item_variant_id'] == '0'){
                        if(isset($ques_value['answer_text']) && !empty($ques_value['answer_text'])){
                        $temp_answer_sheet[$question_set[$ques_value['questionnaire_item_id']]] = $ques_value['answer_text'];
                        } else {
                            $temp_answer_sheet[$question_set[$ques_value['questionnaire_item_id']]] = $ques_value['questionnaire_item_other'];
                        }
                    } else {
                        $variant_table_name = CConfig::get('db.prefix').SurveyQuestionnaireItemVariants::model()->getTableName();
                        $variant = CDatabase::init()->select('
                            SELECT content
                            FROM '.$variant_table_name.'
                            WHERE id = :id',
                            array(
                                ':id'=>$ques_value['questionnaire_item_variant_id']
                            )
                        );
                        $temp_answer_sheet[$question_set[$ques_value['questionnaire_item_id']]] = $variant[0]['content'];
                    }
                }
               
                $answer_sheet[] = $temp_answer_sheet;
            }
            //print_r($answer_sheet);
            $eol = "\r\n";
            $counter = 1;
            
            foreach ($answer_sheet as $single_answer) {
               if($counter == 1){
                    $answer_keys = array_keys($single_answer);
                    $output .= implode(',', $answer_keys);
                    $output .= $eol;
               }
               if(!empty($single_answer)){
                $answer_values = array_values($single_answer);
                $output .= implode(',', $answer_values);
                $output .= $eol;
               }
               $counter++;
            }
    ...
    public function showAction($code = 0)
    {
        ...
        $page = $cRequest->getQuery('page', '', 'questionnaire-welcome');
        $error = $cRequest->getQuery('error');
        $pageSession = $cSession->get('page', 'questionnaire-welcome');
        ...
        if($page != $pageSession){
            ...
            if($validated){
                if($page === 'survey-welcome'){
                    ...
                    $cParticipant = SurveyParticipants::model()->find('survey_id = :survey_id AND phone = :phone', array(':survey_id'=>$survey->id, ':phone'=>CAuth::getLoggedId()));
                   

                    if(CTime::isEmptyDateTime($cParticipant->finish_date)){
                        $cParticipant->finish_date = LocalTime::currentDateTime();
                        $cParticipant->status = 2;
                        $cParticipant->is_active = 1;
                        $cParticipant->save();
                    }
                    ...
                    $newCurrentQuestionnaire = $this->_findNextQuestionnaire($survey->id);
    ...
    public function loginAction($code = 0, $param = '')
    {
        ...
        $this->_view->phone = $phone = $cRequest->getPost('phone');
        $this->_view->gender = $gender = $cRequest->getPost('gender');
        $this->_view->agree = $agree = $cRequest->getPost('agree');
        
        $this->_view->showFirstName = true;
        $this->_view->showLastName = true;
        $this->_view->showEmail = true;
        $this->_view->showPhone = true;
        ...
        if($survey){
            if(!$survey->is_active){
				...
				if(CAuth::isLoggedInAsAdmin()){
					...
					// Set the current questionnaire Id
					if($nextQuestionnaire = $this->_findNextQuestionnaire($survey->id)){
        ...
        if($cRequest->getPost('act') == 'send'){
            ...
            if($fieldPassword == 'allow' && empty($password)){
            ...
            if(!$message){
                ...
                // if($surveyParticipant){                        
                    ...
                    if(!$message){
                        ...
                        /*if($fieldPassword == 'no' || ($fieldPassword == 'allow' && $surveyParticipant->participant_password === $password)){*/
                          ...
                          $this->_setParticipantLogin(
                            ...
                            $nextQuestionnaire = $this->_findNextQuestionnaire($survey->id);
                            ...
                            if($nextQuestionnaire){
                                ...
                                $cParticipant = SurveyParticipants::model()->find('survey_id = :survey_id and phone= :phone', array(':survey_id'=>$survey->id,':phone' => $phone));
								...
								if(!empty($cParticipant)){
                                    ...
                                    $new_participant = new SurveyParticipants();
                                    $new_participant->survey_id = $survey->id;
                                    $new_participant->phone = $phone;
                                    $new_participant->start_date = LocalTime::currentDateTime();
                                    $new_participant->status = 1;
     ...
     * @return int
     */
    private function _findNextQuestionnaire($surveyId)
    {
