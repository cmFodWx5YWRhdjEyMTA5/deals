...
class Uploader {
    ...
    private function prepareFiles(){
        ...
        if($validate){
            ...
            for($i = 0; $i < count($field['name']); $i++){
                ...
                if(!$is_file_removed && $this->validate(array_merge($metas, array('index'=>$i, 'tmp'=>$tmp_name))) && $this->uploadFile($tmp_name, $metas['file'],$metas['name'])){
    ...
    function textwatermark($src, $watermark, $save=NULL) {
        ...
        $new_height = 0;
        $r = $width/$height;
        if ($desired_width/$desired_height > $r) {
            $new_width = $desired_height*$r;
            $new_height = $desired_height;
        } else {
            $new_height = $desired_width/$r;
            $new_width = $desired_width;
        }
        $info = getimagesize($src);
        $extension = image_type_to_extension($info[2]);
        if(isset($extension) && $extension == ".jpeg"){
          $image = imagecreatefromjpeg($src);
        }
        elseif(isset($extension) && $extension == ".png"){
            $image = imagecreatefrompng($src);

        }
        elseif(isset($extension) && $extension == ".gif"){
        ...
        $txtcolor = imagecolorallocate($image_p, 255, 230, 230);
        $font = 'fonts/arial.ttf';
        $font_size = 30;
        $first_text_pos_x = 50;
        $first_text_pos_y = 100;
        $second_text_pos_x = $new_width - 310 - $first_text_pos_x;
        $second_text_pos_y = $new_height - $first_text_pos_y;
        imagettftext($image_p, $font_size, 0, $first_text_pos_x, $first_text_pos_y, $txtcolor, $font, $watermark);
        if($second_text_pos_x > 0 && $second_text_pos_y > 0){
            imagettftext($image_p, $font_size, 0, $second_text_pos_x, $second_text_pos_y, $txtcolor, $font, $watermark);
        }
        if ($save<>'') {
     ...
     * @return boolean
     */
    private function uploadFile($source, $destination,$file_name="",$watermark = true){
        if($this->field['Field_Type'] == 'input'){
            // Bucket Name
            if($this->options['watermark']) {
                $watermark = "bdads24.com"; // Add your own water mark here
                $new_image = $this->textwatermark($source, $watermark, $source);
            }else {
                $new_image = $source;
            }
            $bucket="ad-dwit-a";
            ...
            $s3->putObjectFile($new_image, $bucket , $file_name, S3::ACL_PUBLIC_READ);
            return true;
